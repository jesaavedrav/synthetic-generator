...
class TVAEGenerator(BaseSyntheticGenerator):
    """
    Generates synthetic cardiovascular disease data using TVAE (Tabular VAE)
    """
    ...
    def __init__(self, model_name: str = "cardiovascular_model"):
        ...
        model_filename = get_model_filename(GeneratorMethod.TVAE, model_name)
    ...
    def train(
        ...
        """
        Train the TVAE model on cardiovascular data
        ...
        # Initialize TVAE
        logger.info(f"Initializing TVAE with {epochs} epochs and batch_size {batch_size}...")
        self.model = TVAESynthesizer(
            metadata=self.metadata, epochs=epochs, batch_size=batch_size
        )
...
# Register TVAE generator in factory
GeneratorFactory.register(GeneratorMethod.TVAE, TVAEGenerator)
